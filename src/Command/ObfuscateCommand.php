...
#[AsCommand(name: 'obfuscate:obfuscate', description: 'Obfuscate data')]
class ObfuscateCommand extends Command
{
    public function __construct(
        private readonly Reader $reader,
        private readonly Obfuscate $obfuscate,
    ) {
        parent::__construct();
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->title('Obfuscate data');
        try {
            $config = $this->reader->readConfig($input->getOption('config'));
            $this->obfuscate->obfuscate($config);
