...
final class EmailUnique extends ObfuscateAbstract implements ObfuscateTask
{
    ...
    {
        $this->logger->info('Email Unique generation');
    ...
    protected function processTable(string $table, Definition $definition): void
    {
        ...
        foreach ($this->getRows($table) as $row) {
            if (!$this->isColumnBlank($row[$definition->column] ?? null)) {
                $row[$definition->column] = $this->faker->unique()->safeEmail();
            }
            $this->insertData($row);
            unset($row);
        }
