...
use App\Anonymify\Column\Task\Fax;
...
class FaxTest extends TestCase
{
    ...
    {
        $this->assertSame('fax_no', Fax::getName());
    }
    ...
    public function testRun(): void
    {
        ...
        $resultTables->fetchAllAssociative()->shouldBeCalled()->willReturn([
            'table1' => ['table' => 'table1', 'COLUMN_NAME' => 'fax'],
        ]);
        ...
        $dataResult->iterateAssociative()->shouldBeCalledOnce()->willReturn(
            new \ArrayObject([['fax' => '232423423']])
        );
        $connection->executeQuery('SELECT * FROM table1')
            ->shouldBeCalledOnce()
            ->willReturn($dataResult->reveal());
        $connection->insert('ANONYMIFY_table1', Argument::that(static fn ($data) => array_key_exists('fax', $data)))
        ...
        $definition = new Definition('fax', null);
    ...
    public function testRunWithTableExclusion(): void
    {
        ...
        $resultTables->fetchAllAssociative()->shouldBeCalled()->willReturn([
            'table1' => ['table' => 'table1', 'COLUMN_NAME' => 'fax'],
            'table2' => ['table' => 'table2', 'COLUMN_NAME' => 'fax'],
        ]);
        $connection = $this->prophesize(Connection::class);
        $connection->executeQuery(Argument::that(static fn ($query): bool => str_contains($query, 'information_schema')))
            ->shouldBeCalledOnce()
        ...
        $tableDefinition = new TableDefinitions(['table2' => new Table('table2', ['fax' => null])]);
        $definition = new Definition('fax', null);
        $this->getTask()->run($definition, $tableDefinition);
    }
